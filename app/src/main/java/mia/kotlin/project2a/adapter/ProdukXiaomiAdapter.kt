...
class ProdukXiaomiAdapter (
    val results : ArrayList<ProdukModel.DataXiaomi>
):RecyclerView.Adapter<ProdukXiaomiAdapter.viewHolder>() {
    lateinit var binding: ItemProdukBinding

    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): ProdukXiaomiAdapter.viewHolder {
        binding = ItemProdukBinding.inflate(LayoutInflater.from(parent.context),
            parent,false)
        return viewHolder(binding)
    }

    override fun onBindViewHolder(holder: ProdukXiaomiAdapter.viewHolder, position: Int) {
